...
class CustomDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		# log.debug("i", i)
		data_i = {}
		data_i["image"] = rearrange(self.loader(self.data[i]["image"])['x'][None, ...], 'b x y z -> b z x y')
		t1 = time.time()
		data_i["label"] = rearrange(self.loader(self.data[i]["label"])['x'][None, ...], 'b x y z -> b z x y')
		t2 = time.time()
